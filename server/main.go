...
func getenv(key, def string) string { if v, ok := os.LookupEnv(key); ok && v != "" { return v }; return def }
...
func main() {
    ...
    if err := db.PingContext(ctx); err != nil { log.Error("db ping", "err", err); os.Exit(1) }

    store := NewStore(db)
    if err := store.Migrate(context.Background()); err != nil { log.Error("migrate", "err", err); os.Exit(1) }
    ...
    api := newAPI(store, log); api.routes(mux)
    ...
    ctxSh, cancelSh := context.WithTimeout(context.Background(), 10*time.Second); defer cancelSh()
    if err := srv.Shutdown(ctxSh); err != nil { log.Error("shutdown", "err", err) }
}
