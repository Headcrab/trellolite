...
WORKDIR /src
# Providers like Depot/GCB enforce cache mount IDs to be prefixed with a cache key/namespace.
# Use BUILDKIT_CACHE_MOUNT_NS with a sane default for local builds.
ARG BUILDKIT_CACHE_MOUNT_NS=trellolite
# Copy module manifests first to leverage cache
COPY go.mod go.sum ./
# Use BuildKit caches for modules and build cache
RUN --mount=type=cache,id=${BUILDKIT_CACHE_MOUNT_NS}-gomodcache,target=/go/pkg/mod \
	--mount=type=cache,id=${BUILDKIT_CACHE_MOUNT_NS}-gobuildcache,target=/root/.cache/go-build \
	go mod download
COPY server ./server
